    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100 dark:border-gray-700 transition-all duration-300 hover:shadow-2xl">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                    {{ __('Update the invoice details below.') }}
                                </p>
                            </div>
                        </div>
                ...
                <form action="{{ route('invoices.update', $invoice) }}" method="POST" class="p-8" x-data="{
                    items: {{ Js::from($invoice->items->map(function($item) {
                        return [
                            'title' => $item->title,
                    ...
                    discountRate: {{ $invoice->discount ?? 0 }},
                    discountType: '{{ $invoice->discount_type ?? 'percent' }}',
                    currencySymbol: '{{ $invoice->branch->currency ?? $settings['currency_symbol'] ?? '$' }}',
                    addItem() {
                    ...
                    <div class="space-y-10">
                        ...
                        <section class="relative">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                                <!-- Invoice Number -->
                                <div class="group">
                                    <x-input-label for="invoice_number" :value="__('Invoice Number')" class="mb-2 text-gray-600 dark:text-gray-400 group-focus-within:text-indigo-600 dark:group-focus-within:text-indigo-400 transition-colors" />
                                    <div class="relative">
                                        ...
                                        <x-text-input id="invoice_number" class="block w-full pl-10 bg-gray-50 dark:bg-gray-900/50 border-gray-200 dark:border-gray-700 focus:bg-white dark:focus:bg-gray-900 transition-all" type="text" name="invoice_number" :value="old('invoice_number', $invoice->invoice_number)" required />
                                    </div>
                                        ...
                                        <div class="relative">
                                            ...
                                            <select id="branch_id" name="branch_id" class="block w-full pl-10 bg-gray-50 dark:bg-gray-900/50 border-gray-200 dark:border-gray-700 text-gray-900 dark:text-gray-100 focus:bg-white dark:focus:bg-gray-900 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm transition-all">
                                                ...
                                                @foreach($branches as $branch)
                                                    <option value="{{ $branch->id }}" {{ old('branch_id', $invoice->branch_id) == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                ...
                                <!-- Issue Date -->
                                <div class="group">
                                    <x-input-label for="issue_date" :value="__('Issue Date')" class="mb-2 text-gray-600 dark:text-gray-400 group-focus-within:text-indigo-600 dark:group-focus-within:text-indigo-400 transition-colors" />
                                    <div class="relative">
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            <svg class="h-5 w-5 text-gray-400 group-focus-within:text-indigo-500 transition-colors" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                        ...
                                        <x-text-input id="issue_date" class="block w-full pl-10 bg-gray-50 dark:bg-gray-900/50 border-gray-200 dark:border-gray-700 focus:bg-white dark:focus:bg-gray-900 transition-all" type="date" name="issue_date" :value="old('issue_date', $invoice->issue_date ? \Carbon\Carbon::parse($invoice->issue_date)->format('Y-m-d') : '')" required />
                                    </div>
                                ...
                                <!-- Due Date -->
                                <div class="group">
                                    <x-input-label for="due_date" :value="__('Due Date')" class="mb-2 text-gray-600 dark:text-gray-400 group-focus-within:text-indigo-600 dark:group-focus-within:text-indigo-400 transition-colors" />
                                    <div class="relative">
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            <svg class="h-5 w-5 text-gray-400 group-focus-within:text-indigo-500 transition-colors" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                                <path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd" />
                                        ...
                                        <x-text-input id="due_date" class="block w-full pl-10 bg-gray-50 dark:bg-gray-900/50 border-gray-200 dark:border-gray-700 focus:bg-white dark:focus:bg-gray-900 transition-all" type="date" name="due_date" :value="old('due_date', $invoice->due_date ? \Carbon\Carbon::parse($invoice->due_date)->format('Y-m-d') : '')" required />
                                    </div>
                            ...
                            <!-- Recurring Options -->
                            <div class="mt-6 p-4 bg-gray-50 dark:bg-gray-700/30 rounded-lg border border-gray-100 dark:border-gray-700" x-data="{ isRecurring: {{ $invoice->is_recurring ? 'true' : 'false' }} }">
                                <div class="flex items-center">
                                    <input id="is_recurring" type="checkbox" name="is_recurring" value="1" x-model="isRecurring" class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 dark:focus:ring-indigo-600 dark:ring-offset-gray-800 dark:bg-gray-700 dark:border-gray-600">
                                ...
                                <div x-show="isRecurring" x-transition class="mt-4">
                                    <x-input-label for="recurring_frequency" :value="__('Frequency')" class="mb-2 text-gray-600 dark:text-gray-400" />
                                    <select id="recurring_frequency" name="recurring_frequency" class="block w-full md:w-1/3 bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-700 text-gray-900 dark:text-gray-100 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm transition-all">
                                        ...
                                        <option value="semi_annually" {{ old('recurring_frequency', $invoice->recurring_frequency) == 'semi_annually' ? 'selected' : '' }}>Semi-Annually</option>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                            <div class="space-y-8">
                                <div class="group">
                                    ...
                                    <textarea id="notes" name="notes" rows="4" class="block w-full bg-gray-50 dark:bg-gray-900/50 border-gray-200 dark:border-gray-700 text-gray-900 dark:text-gray-100 focus:bg-white dark:focus:bg-gray-900 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm transition-all" placeholder="Add any notes for the client...">{{ old('notes', $invoice->notes) }}</textarea>
                                </div>
                                <div class="group">
                                    <x-input-label for="terms" :value="__('Terms & Conditions')" class="mb-2 text-gray-600 dark:text-gray-400" />
                                    <textarea id="terms" name="terms" rows="4" class="block w-full bg-gray-50 dark:bg-gray-900/50 border-gray-200 dark:border-gray-700 text-gray-900 dark:text-gray-100 focus:bg-white dark:focus:bg-gray-900 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm transition-all" placeholder="Add terms and conditions...">{{ old('terms', $invoice->terms) }}</textarea>
                                </div>
                            </div>
                        ...
                        <div class="flex items-center justify-end mt-8 border-t border-gray-100 dark:border-gray-700 pt-6">
                            <a href="{{ route('invoices.index') }}" class="text-gray-700 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 mr-6 font-medium" wire:navigate>Cancel</a>
            ...
            if (branchSelect) {
                branchSelect.addEventListener('change', function() {
                    ...
                    if (branchId) {
                        ...
                        // Fetch Projects
                        if (projectSelect) {
                            fetch(`/branches/${branchId}/projects`)
                                .then(response => response.json())
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const branchSelect = document.getElementById('branch_id');
